...
foreach ($site in $sites) {
    ...
    if ($siteprops.GroupId.Guid -ne '00000000-0000-0000-0000-000000000000') {
        ...
        foreach ($groupowner in $groupowners) {
            ...
            $ExportItem1  | Add-Member -MemberType NoteProperty -name "Entra Group Owners" -value ($($gowner.DisplayName) -join ',')
            $ExportItem1  | Add-Member -MemberType NoteProperty -name "Entra Group Owners Email" -value ($($gowner.PrimarySmtpAddress) -join ',')
            $ExportItem1  | Add-Member -MemberType NoteProperty -name "Entra Groups Owners InfoSegment" -value ($($gowner.InformationBarrierSegments) -join ',')
            $output1 += $ExportItem1
        }   
        ...
        foreach ($groupmember in  $groupmembers) {
            ...
            $ExportItem2 = New-Object PSObject 
            $ExportItem2  | Add-Member -MemberType NoteProperty -name "Entra Group Members" -value ($($gmember.DisplayName) -join ',')
            $ExportItem2  | Add-Member -MemberType NoteProperty -name "Entra Group Members Email" -value ($($gmember.PrimarySmtpAddress) -join ',')
            $ExportItem2  | Add-Member -MemberType NoteProperty -name "Entra Groups Member InfoSegment" -value ($($gmember.InformationBarrierSegments) -join ',')
            $output2 += $ExportItem2
        ...
        $ExportItem3 = New-Object PSObject 
        $ExportItem3  | Add-Member -MemberType NoteProperty -name "Entra Group Displayname" -value ($($AADGroups.DisplayName))
        $ExportItem3  | Add-Member -MemberType NoteProperty -name "Entra Group Alias" -value ($($AADGroups.Alias) -join ',')
        $ExportItem3  | Add-Member -MemberType NoteProperty -name "Entra Group AccessType" -value ($($AADGroups.AccessType) -join ',')
        $ExportItem3  | Add-Member -MemberType NoteProperty -name "Entra Group ManagedBy" -value ($($AADGroups.ManagedBy) -join ',')
        $ExportItem3  | Add-Member -MemberType NoteProperty -name "Entra Group WhenCreated" -value ($($AADGroups.WhenCreated) -join ',')
        $output3 += $ExportItem3
    }
    ...
    ForEach ($Group in $Groups) {
        ...
        ForEach ($GroupUser in $Group.Users) {
            ...
            $ExportItem4 = New-Object PSObject 
            $ExportItem4  | Add-Member -MemberType NoteProperty -name "SPGroup Title" -value ($($Group.Title))
            $ExportItem4  | Add-Member -MemberType NoteProperty -name "SPGroup Roles" -value ($($Group.Roles) -join ',')
            $ExportItem4  | Add-Member -MemberType NoteProperty -name "SPGroup Users" -value ($($Group.Users) -join ',')
            $ExportItem4  | Add-Member -MemberType NoteProperty -name "SP Group User Name" -value ($($guser.DisplayName))
            $ExportItem4  | Add-Member -MemberType NoteProperty -name "SP Group User E-mail" -value ($($guser.PrimarySmtpAddress) -join ',')
            $ExportItem4  | Add-Member -MemberType NoteProperty -name "SP Group User InfoSegment" -value ($($guser.InformationBarrierSegments) -join ',')
            $output4 += $ExportItem4
        }
...
$allexport = $output + $output1 + $output2 + $output3 + $output4
$allexport | Export-Csv $outputfile -NoTypeInformation -Append   
